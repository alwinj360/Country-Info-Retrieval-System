...
class CountryIRSystemSLDict(CountryIRSystemAbstract):
  def __init__(self):
    self._cInventory = SLDict()
  ...
  def search_country(self, country_name):
    # optimize through binary search?
  ...
  def series_for_given_country(self, inp, strg_country):
    """ Based on user input, a number from 1 to 5, and a country name,
      the function identifies the corresponding development indicator 
      and retrieves related information if the country data is found. If not,
      none is returned.
  ...
  # Author: Vel 
  # modified from version 7 algorithm
